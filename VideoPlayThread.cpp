...
{
    displayVideo();
    Log("Video play thread exit.");
    m_state = THREAD_STATE::FINISHED;
}

void VideoPlayThread::displayVideo()
{
    // // std::ofstream outputFile("videoTimeDiff.csv");
    // std::ofstream outputFile1("videoClock.csv");
    // std::ofstream outputFile2("audioClock.csv");
    // std::ofstream outputFile3("videoTimeDiff.csv");

    while (true) {
        if (m_state == THREAD_STATE::STOPPED) break;
        if (m_state == THREAD_STATE::PAUSED) continue;
